...
static ssize_t lcm_write(struct file *filp, const char *buf, size_t len, loff_t *off)
{
	...
	printk(KERN_ALERT "%s, %d, %d", buf, len, (int)*off);
	
	for (i=0; i<len; i++) {
		if (pos + i > 31) {
			break;
		}
		lcm_set_word_pos(buf[i], pos + i);
	}
...
static long lcm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case LCM_W_MOD:
			if (copy_from_user(&lcm_cmd, (int __user *)arg, sizeof(uint8_t))) {
				retval = -EFAULT;
				goto done;
			}
			
			lcm_set_cmd(lcm_cmd);
			if (lcm_cmd == 1 || lcm_cmd == 2) {
				mdelay(3);
			} else {
				udelay(100);
			}
			
			printk(KERN_ALERT "lcm cmd is %d\n", lcm_cmd);
			break;
